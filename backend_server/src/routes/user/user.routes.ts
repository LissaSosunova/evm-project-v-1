...
import { RefreshAccessToken } from '../../controllers/user/refresh_token';
...
userRoutes.post('/change_password_auth',
                [verifyToken],
                new ChangePasswordAuthController().changePassword.bind(new ChangePasswordAuthController()));
...
userRoutes.delete('/delete_contact/:contactUsername',
                    [verifyToken],
                    new DeleteContactController().delete.bind(new DeleteContactController()));
...
userRoutes.post('/upload_avatar',
                [verifyToken],
                new FileUpload().upload.single('image'), new UploadAvatarController().uploadAvatar.bind(new UploadAvatarController()));
userRoutes.get('/refresh_token', new RefreshAccessToken().getNewToken);
